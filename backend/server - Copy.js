...
// 1. LOGIN (Check against USERS array)
app.post('/api/login', (req, res) => {
    let { username, password } = req.body;
    
    // 👇 UPDATED: Capitalize First Letter of EVERY Word (e.g. "ahmed atef" -> "Ahmed Atef")
    if(username) {
...
// 2. CREATE USER (From Dashboard)
app.post('/api/users', (req, res) => {
    let { username, password } = req.body;
    if (!username || !password) return res.json({ success: false });
    
    // 👇 UPDATED: Capitalize First Letter of EVERY Word
    username = username.trim().toLowerCase()
        .split(' ')
        .map(word => word.charAt(0).toUpperCase() + word.slice(1))
        .join(' ');
...
app.delete('/api/users/:username', (req, res) => {
    ...
    // Prevent deleting the last user so you don't get locked out
...
// 8. LOGISTICS HUB (HTML DASHBOARD)
app.get('/api/orders', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT id, ship_name, status, signature_data, report_text, delivery_date 
      FROM orders 
            ...
            ${pendingShips.map(s => `
                <div class="card">
                    ...
                    <div style="margin-top:10px; font-size:11px;">
                        ${s.items.map(i => `<div>${i.name} <b style="color:#FFC107">x${i.qty}</b></div>`).join('')}
                    </div>
                </div>
            `).join('')}
        </div>
