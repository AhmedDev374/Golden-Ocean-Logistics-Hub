...
import { 
  Package, Ship, Lock, LogIn, LogOut, 
  Anchor, Clock, CheckCircle, ShieldCheck, Award 
} from 'lucide-react';
...
function App() {
  ...
  return (
    <div className="mobile-container">
      {/* --- NEW: CERTIFICATE MODAL (Mimics Server.js Design) --- */}
      {certModalItem && (
        <div style={{
          position:'fixed', zIndex:999, left:0, top:0, width:'100%', height:'100%', 
          background:'rgba(0,0,0,0.9)', display:'flex', alignItems:'center', justifyContent:'center', padding:'20px'
        }} onClick={() => setCertModalItem(null)}>
          <div style={{
            background:'white', color:'black', width:'100%', maxWidth:'400px', padding:'30px', 
            border:'10px double #c5a059', textAlign:'center', boxShadow:'0 0 50px rgba(255,193,7,0.3)'
          }} onClick={e => e.stopPropagation()}>
            <h2 style={{color:'#c5a059', margin:0, fontSize:'18px'}}>ISO 9001:2015</h2>
            <p style={{fontSize:'10px', letterSpacing:'2px', margin:'5px 0'}}>CERTIFICATE OF CONFORMITY</p>
            <hr style={{border:'1px solid #c5a059', width:'50%', margin:'15px auto'}} />
            <p style={{marginTop:'20px', fontSize:'14px'}}>This is to certify that the product:</p>
            <h2 style={{textTransform:'uppercase', margin:'10px 0', fontSize:'22px'}}>{certModalItem}</h2>
            <p style={{fontSize:'12px', color:'#555'}}>has been inspected and found to be in compliance with international maritime safety standards.</p>
            <div style={{marginTop:'30px', display:'flex', justifyContent:'space-around'}}>
                <div>
                    <p style={{fontSize:'8px', margin:0}}>DATE OF ISSUE</p>
                    <b style={{fontSize:'12px'}}>{new Date().toLocaleDateString()}</b>
                </div>
                <div>
                    <p style={{fontSize:'8px', margin:0}}>AUTHORITY</p>
                    <b style={{fontSize:'12px'}}>GOLDEN OCEAN HUB</b>
                </div>
            </div>
            <button 
              onClick={() => setCertModalItem(null)} 
              style={{marginTop:'30px', background:'#c5a059', color:'white', border:'none', padding:'10px 20px', borderRadius:'4px', cursor:'pointer'}}
            >Close</button>
          </div>
        </div>
      )}
        ...
        <div>
            ...
            {filteredShips.map(ship => (
                <div key={ship.id} 
                    ...
                    <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: '6px', background: ship.status==='PENDING' ? '#FFC107' : '#4CAF50' }}></div>
                    ...
                    <div style={{display:'flex', justifyContent:'space-between', alignItems:'start', marginBottom:'8px', paddingLeft:'10px'}}>
                        ...
                        <div style={{
                            ...
                            display: 'flex', alignItems: 'center', gap: '5px'
                             ...
                             {ship.status === 'PENDING' ? <><Clock size={12}/> PENDING</> : <><CheckCircle size={12}/> DELIVERED</>}
                        </div>
                    </div>
                    ...
                    <div style={{ marginTop:'15px', paddingTop:'10px', borderTop:'1px solid #333', paddingLeft:'10px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                        <div style={{fontSize:'13px', color:'#777'}}>📦 {ship.items.length} Items</div>
                        {ship.status === 'DELIVERED' && <div style={{color:'#4CAF50', fontSize:'13px'}}>Completed</div>}
                    </div>
                </div>
            ))}
        </div>
        ...
        <div className="card">
            ...
            <div style={{marginBottom:'10px'}}>
                {selectedShip.items.map((item, i) => (
                    <div key={i} style={{background:'#2a2a2a', padding:'12px', marginBottom:'5px', borderRadius:'6px', display:'flex', justifyContent:'space-between', alignItems: 'center'}}>
                        <div style={{display:'flex', alignItems:'center', gap:'8px'}}>
                            <span>{item.name}</span>
                            {/* --- ADDED CERTIFICATE ICON CHECK --- */}
                            {item.cert && (
                              <Award 
                                size={16} 
                                color="#FFC107" 
                                style={{cursor:'pointer'}} 
                                onClick={() => setCertModalItem(item.name)}
                              />
                            )}
                        </div>
            ...
            <div style={{
                ...
                <div style={{background:'#4CAF50', padding:'8px', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center'}}>
                    <ShieldCheck size={20} color="white" />
                </div>
                <div>
                    <div style={{color:'#4CAF50', fontWeight:'bold', fontSize:'14px'}}>Certified Manifest</div>
                    <div style={{color:'#888', fontSize:'11px'}}>All items verified for safety & compliance</div>
                </div>
            </div>
      ...
      {screen === 'SIGN' && selectedShip && (
        <div className="card">
            ...
            <textarea 
                ...
                style={{
                    width:'100%', height:'100px', background:'#1a1a1a', border:'1px solid #333', 
                    color:'white', padding:'15px', borderRadius:'8px', boxSizing:'border-box', 
                    marginBottom:'20px', fontSize: '14px', resize: 'none'
                }}
            ></textarea>
